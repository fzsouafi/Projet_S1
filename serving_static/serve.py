...
from flask import render_template, jsonify, current_app, request, make_response, flash
from function_search_taxonomy import *
import sys
...
# routes dans lesquelles est affiché un message dans la console ( message d'erreur ou alors le dictionnaire contenant les informations de l'espèce)
...
def create_entry():
    ...
    if req['scientiesp'] != "-":
        ...
        file.write(s + taxonomic_dico[req['scientiesp']][0] + s + v + s + taxonomic_dico[req['scientiesp']][5] + s + v + s
                   ...
                   + s + v + s + " https://www.ncbi.nlm.nih.gov/Taxonomy/Browser/wwwtax.cgi?id="+taxonomic_dico[req['scientiesp']][4] + s + v + s + taxonomic_dico[req['scientiesp']][0]+'"' + '\n')
        file.close()
        createIndex()
        ...
        file.write( s + taxonomic_dico[req['namaesp']][0] + s + v + s + taxonomic_dico[req['namaesp']][5] + s + v + s
                    ...
                    + s + v + s + taxonomic_dico[req['namaesp']][3] + s + v + s + taxonomic_dico[req['namaesp']][4] + s + v + s
                    + " https://www.ncbi.nlm.nih.gov/Taxonomy/Browser/wwwtax.cgi?id="+taxonomic_dico[req['namaesp']][4] + s + v + s + taxonomic_dico[req['namaesp']][0]+'"'+ '\n')
        file.close()
        createIndex()
...
def create_association():
    ...
    s = "\""
    v = ","
    file.write(s + req["espSource"] + s + v + s + req["espInteraction"] + s + v + s + req["espCible"] + s + v + v + v + "\n")
    file.close()
    createIndex()
    return req
...
# les fonctions de base du serveur et une fonction d'execution automatique de Monpotager.py
